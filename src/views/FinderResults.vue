    ...
    <main id="main-content" class="nhsuk-main-wrapper">
      <div class="nhsuk-u-reading-width">
        <div class="nhsuk-back-link">
          <router-link to="/find-by-location">
            <svg
        ...
        <div v-if="mhResults">
          ...
          <h2 class="nhsuk-body-l">
            <span role="text">
              <span class="results__count">{{ mhResults.length }}</span>
              service(s) are available for location.
            </span>
          </h2>

<p class="small-test-note" v-if="mhTestCase">DBG: {{ mhTestCase }}</p>
<p class="small-test-note" v-if="postCodeResult">DBG: {{ postCodeResult }}</p>
          ...
          <div>
            <ol class="nhsuk-list inline-list">
              <li
                v-for="mhOption in mhResults"
                :key="mhOption.OrganisationName"
                class="nhsuk-c-panel results__item"
                data-index="1"
              >
                <hr />
                <div>
                  <h3 class="results__name">{{ mhOption.OrganisationName }}</h3>
                  <p class="results__website">
                    <a href="#"
                      >Visit<span class="nhsuk-u-visually-hidden">{{
                        mhOption.OrganisationName
                      }}</span
                      >website</a
                    >
                  </p>
                  ...
                  <p class="results__telephone">
                    Telephone:<a href="#" class="telephone_tracking">{{
                      mhOption.phone
                    }}</a>
                  </p>
                  ...
                  <p class="results__email">
                    Email:<a
                      href="mailto:hillingdontalkingtherapies.cnwl@nhs.net"
                      >{{ mhOption.email }}</a
                    >
                  </p>
                    ...
                    <a class="nhsuk-action-link__link" href="#">
                      ...
                      <span class="nhsuk-action-link__text"
                        >Refer yourself online<span
                          ...
                          >{{ mhOption.OrganisationName }}</span
                        >
                      </span>
                    </a>
                  </div>
...
export default {
  name: "FinderResults",
  computed: {
    ...mapState("search", ["postCodeResult", "mhTestCase", "mhResults"]),
  },
};
</script>
